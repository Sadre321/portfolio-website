...
const ProjectItems = ({ repo }) => {
  return (
    <li className="p-6 border-l-4 border-[#D4A373] rounded-lg shadow-md hover:shadow-xl transition-all duration-300 w-full sm:w-64 md:w-80 lg:w-96 min-h-[180px] bg-white flex flex-col justify-between">
      ...
      {/* Repo Açıklaması */}
      <p className="text-gray-700 mb-4 md:text-lg text-base flex-grow">
        {repo.description || "Bu repo hakkında açıklama bulunmamaktadır."}
      </p>

      {/* Repo İstatistikleri (Yıldızlar, Forklar vb.) */}
      <div className="flex items-center md:space-x-4 space-x-2 text-sm text-gray-600">
        <div className="flex items-center">
          ...
          <span className="md:ml-1">{repo.stargazers_count}</span> Stars
        </div>
        <div className="flex items-center">
          <svg
            xmlns="http://www.w3.org/2000/svg"
          ...
          <span className="md:ml-1">{repo.forks_count}</span> Forks
        </div>
      </div>
    </li>
  );
